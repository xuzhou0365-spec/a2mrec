...
class GRU4Rec(nn.Module):
    ...
    def forward(self, item_seq, augment_sequence=None, alpha=None, perturbed=False):
        item_seq_emb = self.item_embeddings(item_seq)

        if augment_sequence is not None:
            item_embeddings_org = self.item_embeddings(item_seq)
            item_embeddings_aug = self.item_embeddings(augment_sequence)
            item_seq_emb = alpha * item_embeddings_org + (1 - alpha) * item_embeddings_aug
