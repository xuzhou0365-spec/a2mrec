...
from models import MaskGenerator
...
class Trainer:
    def __init__(self, model, train_dataloader,
        ...
        self.maskgenerater = MaskGenerator(args)

        if self.cuda_condition:
            self.model.cuda()
            self.maskgenerater.cuda()
        ...
        self.optim_adv = Adam(self.maskgenerater.parameters(), lr=self.args.lr_adv, weight_decay=self.args.weight_decay)
    ...
    def cross_entropy_rec(self, seq_out, pos_ids, neg_ids, pos_emb, neg_emb, weight=None):
...
class SASRecTrainer(Trainer):
    ...
    def iteration(self, epoch, dataloader, full_sort=True, train=True):
        ...
        if train:
            ...
            for i, (rec_batch, aug_batches) in rec_cf_data_iter:
                ...
                users_emb = sequence_output
                pos_emb = self.model.item_embeddings(target_pos)
                neg_emb = self.model.item_embeddings(target_neg)
                rec_loss = self.cross_entropy_rec(sequence_output, target_pos, target_neg, pos_emb, neg_emb)
                ...
                if epoch > 100:
                    delta_users_r, delta_pos_items_r, delta_neg_items_r = \
                        torch.autograd.grad(rec_loss.mean(), (users_emb, pos_emb, neg_emb), retain_graph=True)
                    delta_users_r = torch.nn.functional.normalize(delta_users_r, p=2, dim=-1) * self.eps
                    delta_pos_items_r = torch.nn.functional.normalize(delta_pos_items_r, p=2, dim=-1) * self.eps
                    delta_neg_items_r = torch.nn.functional.normalize(delta_neg_items_r, p=2, dim=-1) * self.eps

                    new_useremb = users_emb + delta_users_r
                    new_posemb = pos_emb + delta_pos_items_r
                    new_negemb = neg_emb + delta_neg_items_r
                    ...
                    joint_loss = joint_loss + 0.3 * self.cross_entropy_rec(new_useremb, target_pos, target_neg, new_posemb, new_negemb )
                    #0.5
                # ---------- augmentation -------------#
                aug_losses = []
                if epoch >= self.args.pretrain_epoch:
                    ...
                    for aug_batch in aug_batches:
                        # aug_loss = self.aug_mix_learning_adv(input_ids, aug_batch, target_pos, target_neg)
                    ...
                    if epoch > 100:
                        ...
                        # aug_loss = self.aug_mix_learning_adv1(input_ids,  target_pos, target_neg)
                        # aug_losses.append(aug_loss)
                        # joint_loss += self.args.aml_weight * aug_loss

                        mask_output = self.maskgenerater.masktransformer.transformer_encoder(input_ids)
                        whole_mix_loss_adv = self.whole_mix_learning_adv(sequence_output, mask_output, target_pos, target_neg, input_ids)
                        joint_loss +=  whole_mix_loss_adv
                ...
                if epoch > 100: # self.args.pretrain_epoch:
                    self.maskgenerater.train()
                    self.model.eval()

                    maximize_whole_mix_loss = 0
                    # aug_loss = self.aug_mix_learning_adv1(input_ids, target_pos, target_neg)
                    # #aug_losses.append(aug_loss)
                    # maximize_whole_mix_loss += maximize_whole_mix_loss + self.args.aml_weight * aug_loss * (-1)

                    # for aug_batch in aug_batches:
                    #     aug_loss = self.aug_mix_learning_adv1(input_ids, aug_batch, target_pos, target_neg)
                    #     #aug_losses.append(aug_loss)
                    #     maximize_whole_mix_loss += maximize_whole_mix_loss + self.args.aml_weight * aug_loss * (-1)
                    ...
                    whole_mix_loss_adv = self.whole_mix_learning_adv(sequence_output, mask_output, target_pos, target_neg, input_ids)
                    maximize_whole_mix_loss += whole_mix_loss_adv * (-1)

                    self.optim_adv.zero_grad()
                    maximize_whole_mix_loss.backward()
                    self.optim_adv.step()
    ...
    def whole_mix_learning_adv(self, seq_out, mask_out, pos_ids, neg_ids, input_ids):
        ...
        # for i in range(1):  #self.n_whole_level
        #     #alpha_beta = self.beta_distribution.sample((seq_out.shape[0], seq_out.shape[1])).to(self.device)  #(B, N, 1)
        #
        ...
        #     CC = mask_out[indices]
        #
        #     alpha = self.maskgenerater(torch.cat((AA, CC),dim=-1), flag=1)  #(B, N, 1)
        #
        #
        #
        ...
        for i in range(1):#self.n_whole_level
            #alpha_beta = self.beta_distribution.sample((seq_out.shape[0], seq_out.shape[2])).permute(0, 2, 1).to(self.device)  ##(B, 1, D)
            indices = torch.randperm(seq_out.shape[0])
            ...
            alpha = self.maskgenerater(torch.cat((AA, CC), dim=-1), flag=2)  #(B, N, D)
            # alpha_beta = self.beta_distribution.sample((seq_out.shape[0], seq_out.shape[2])).permute(0, 2, 1).to(self.device)
            # alpha = weightadv * alpha + (1-weightadv) * alpha_beta
    ...
    def whole_mix_learning(self, seq_out, pos_ids, neg_ids, input_ids):
        ...
        for i in range(self.n_whole_level):
            ...
            mixed_seq_out = alpha_beta * seq_out + (1 - alpha_beta) * seq_out[indices]



            mixed_pos_emb = alpha_beta * pos_emb + (1 - alpha_beta) * pos_emb[indices]
            mixed_neg_emb = alpha_beta * neg_emb + (1 - alpha_beta) * neg_emb[indices]
            mix_pos_logits = (mixed_seq_out * mixed_pos_emb).sum(dim=-1).view(-1)
            mix_neg_logits = (mixed_seq_out * mixed_neg_emb).sum(dim=-1).view(-1)
    ...
    def aug_mix_learning(self, input_ids, aug_inputs, target_pos, target_neg):
        ...
        for aug_batch in aug_inputs:
            ...
            augment_output = self.model.transformer_encoder(input_ids, aug_sequences, alpha=mix_weight)
            aug_loss += self.cross_entropy(augment_output, target_pos, target_neg, loss_weight)
        return aug_loss

    def aug_mix_learning_adv1(self, input_ids, target_pos, target_neg):
        """
        inputs: [batch1_augmented_data, batch2_augmentated_data]
        """
        aug_loss = 0

        augment_output = self.model.transformer_encoder(input_ids, maskgenerator=self.maskgenerater)
        aug_loss += self.cross_entropy(augment_output, target_pos, target_neg)
