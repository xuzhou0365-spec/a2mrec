...
from modules import Encoder, LayerNorm, ACT2FN, gelu, swish
...
class SASRecModel(nn.Module):
    ...
    def add_position_embedding(self, sequence, augment_sequence=None, alpha=None, maskgenerator=None):
        ...
        if maskgenerator is not None:
            alpha_input1 = maskgenerator.masktransformer2.transformer_encoder(sequence)
            # alpha_input2 = maskgenerator.masktransformer2.transformer_encoder(augment_sequence)
            # input_cat = torch.cat((alpha_input1, alpha_input2), dim=-1)
            input_cat = torch.cat((alpha_input1, alpha_input1), dim=-1)
            alpha_mask = maskgenerator(input_cat, flag=3)
            item_embeddings = alpha_mask * item_embeddings


        elif augment_sequence is not None:
            item_embeddings_org = self.item_embeddings(sequence)
            item_embeddings_aug = self.item_embeddings(augment_sequence)

            if alpha is not None:
                 alpha_mask = alpha
                 item_embeddings = alpha_mask * item_embeddings_org   + (1 - alpha_mask) * item_embeddings_aug
    ...
    def transformer_encoder(self, input_ids, augment=None, alpha=None, perturbed=False, maskgenerator=None):
        ...
        if alpha is not None:
           sequence_emb = self.add_position_embedding(input_ids, augment, alpha=alpha)
        elif maskgenerator is not None:
            sequence_emb = self.add_position_embedding(input_ids, maskgenerator= maskgenerator)
        else:
...
class MaskGenerator(nn.Module):
    def __init__(self, args):
        ...
        self.masktransformer = SASRecModel(args)
        self.masktransformer2 = SASRecModel(args)

        self.MG1 = MaskGenerator1(args)
        self.MG2 = MaskGenerator2(args)
        self.MG3 = MaskGenerator3(args)


    def forward(self, input_tensor, flag):

        if(flag == 1):
            hidden_states = self.MG1(input_tensor)
        elif (flag==2):
            hidden_states = self.MG2(input_tensor)
        elif (flag==3):
            hidden_states = self.MG3(input_tensor)



        return hidden_states

# class MaskGenerator1(nn.Module):
#     def __init__(self, args):
#         super(MaskGenerator1, self).__init__()
#         self.dense_1 = nn.Linear(args.hidden_size * 2 , args.hidden_size)
#         if isinstance(args.hidden_act, str):
#             self.intermediate_act_fn = ACT2FN[args.hidden_act]
#         else:
#             self.intermediate_act_fn = args.hidden_act
#
#         self.dense_2 = nn.Linear(args.hidden_size, 1)
#         #self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12)
...
#
#         hidden_states = self.sigmoid(hidden_states)   #(B, N, 1)
#
#         return hidden_states
#
# class MaskGenerator3(nn.Module):
#     def __init__(self, args):
#         super(MaskGenerator3, self).__init__()
#         self.dense_1 = nn.Linear(args.hidden_size * 2 , args.hidden_size)
#         if isinstance(args.hidden_act, str):
#             self.intermediate_act_fn = ACT2FN[args.hidden_act]
#         else:
...
#
#
# class MaskGenerator2(nn.Module):
#     def __init__(self, args):
#         super(MaskGenerator2, self).__init__()
#         self.dense_1 = nn.Linear(args.hidden_size * 2 , args.hidden_size)
#         if isinstance(args.hidden_act, str):
#             self.intermediate_act_fn = ACT2FN[args.hidden_act]
#         else:
...
#         #hidden_states = self.LayerNorm(hidden_states + input_tensor)
#
#         hidden_states = self.sigmoid(hidden_states)   #(B, N, D)
#
#         return hidden_states
...
class MaskGenerator1(nn.Module):
    def __init__(self, args):
        super(MaskGenerator1, self).__init__()
        self.mixupencoder = Encoder(args)
